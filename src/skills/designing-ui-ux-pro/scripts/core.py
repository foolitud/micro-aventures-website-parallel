...
class BM25:
    ...
    def score(self, query: str) -> list[tuple[int, float]]:
        ...
        for idx, doc in enumerate(self.corpus):
            score: float = 0.0
            doc_len: int = self.doc_lengths[idx]
            ...
            for token in query_tokens:
                if token in self.idf:
                    tf: float = float(term_freqs.get(token, 0))
                    idf: float = self.idf[token]
                    numerator: float = tf * (self.k1 + 1)
                    # Guard against zero avgdl
                    avgdl: float = self.avgdl if self.avgdl > 0 else 1.0
                    denominator: float = tf + self.k1 * (1 - self.b + self.b * doc_len / avgdl)
...
def detect_domain(query):
    ...
    for domain, keywords in domain_keywords.items():
        score: int = 0
        for kw in keywords:
            # Use word boundaries only for purely alphanumeric keywords
