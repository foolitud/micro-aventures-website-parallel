...
This is the most important concept to understand before building skills. Where you put instructions determines how the agent uses them.
...
**Rule of thumb:** If the agent should *always* know it, put it in CLAUDE.md. If the agent should only know it when doing a specific task, make it a skill.
...
|-------|----------|------|---------|-------------|
| `name` | No | string | directory name | Display name and `/slash-command`. Gerund form recommended. Lowercase letters, numbers, hyphens only. Max 64 chars. |
| `description` | Recommended | string | first paragraph of content | What the skill does and when to use it. The agent uses this for auto-invocation decisions. |
...
| `disable-model-invocation` | No | boolean | `false` | When `true`, only the user can invoke the skill. Removes it from the agent's context entirely. |
| `user-invocable` | No | boolean | `true` | When `false`, hides the skill from the `/` menu. Only the agent can invoke it. |
...
- **Skills main docs:** https://antigravity.google/docs/skills
- **Subagents:** https://antigravity.google/docs/subagents
- **Memory (CLAUDE.md):** https://antigravity.google/docs/memory
